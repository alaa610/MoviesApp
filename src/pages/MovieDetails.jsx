...
const MovieDetails = () => {
    ...
    useEffect(() => {
        ...
        getRecommendedMovies(id).then((data) => setRecommendations(data.results || []));
        window.scrollTo(0, 0);
    }, [id]);
    ...
    return (
        <div className="movie-details bg-light p-4 rounded">
            {/* Movie info */}
            <div className="d-flex flex-column flex-md-row gap-4">
                <div className="col-md-4">
                    <img src={getImageUrl(movie.poster_path, "w500")} alt={movie.title} className="img-fluid rounded shadow-sm" />
                </div>
                <div className="col-md-8">
                    <h2 className="fw-bold">{movie.title}</h2>
                    <p className="text-muted opacity-50">{formatDate(movie.release_date)}</p>
                    ...
                    <div className="mt-3 d-flex flex-column justify-content-start align-items-start gap-2">
                        <img src={getImageUrl(movie.production_companies[0].logo_path, "w200")} alt="Studio Logo" height={40} />
                        <a href={movie.homepage} className="btn btn-outline-dark btn-sm mt-2" target="_blank" rel="noopener noreferrer">
                            Website 🔗
                        </a>
                    </div>
                </div>
            ...
            <div className="row">
                {recommendations.slice(0, 6).map((rec) => {
                    ...
                    const scoreColor = score >= 75 ? "bg-success" : score >= 60 ? "bg-warning text-dark" : "bg-danger";

                    return (
                        <div key={rec.id} className="col-6 col-sm-4 col-md-3 col-lg-2 mb-4">
                            <Link to={`/movie/${rec.id}`} className="text-decoration-none text-dark">
                                <div
                                    ...
                                    {/* Poster */}
                                    <img src={getImageUrl(rec.poster_path, "w200")} className="card-img-top mb-4" alt={rec.title} style={{ height: "230px", objectFit: "cover" }} />

                                    {/* Title + Date */}
                                    <div className="card-body p-2">
                                        <h6 className="card-title mb-1" style={{ fontSize: "0.85rem", fontWeight: "600" }}>
                                            {rec.title?.length > 20 ? rec.title.slice(0, 20) + "..." : rec.title}
                                        </h6>
                                        <small className="text-muted">{formatDate(rec.release_date)}</small>
                                    </div>
                                </div>
                            </Link>
                        </div>
